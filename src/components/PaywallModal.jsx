...
export default function PaywallModal({ isOpen, onClose }) {
  ...
  const checkout = async (plan) => {
    ...
    try { await openBilling('checkout', { plan }); } catch (err) { setError(err.message); setBusy(false); }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-fadeIn select-none bg-black/80 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        <button aria-label="Fechar" onClick={onClose} className="absolute top-4 right-4 p-2 bg-black/5 rounded-full hover:bg-black/10 transition-colors z-10 text-black">
          <X size={20} />
        </button>

        {error && <p role="alert" className="absolute top-12 left-4 right-4 bg-red-100 p-3 text-black">{error}</p>}
        ...
        <div className="flex-1 p-8 md:p-12 border-b-4 md:border-b-0 md:border-r-4 border-black bg-white flex flex-col justify-between">
          ...
          <button 
            disabled={busy} onClick={() => checkout('base')}
        ...
        <div className="flex-1 p-8 md:p-12 bg-yellow-400 flex flex-col justify-between">
          <div>
            ...
            <ul className="space-y-4 mb-8">
              ...
              <li className="flex items-start gap-3 text-sm font-bold text-black"><Check size={18} className="text-black shrink-0 mt-0.5" /> Preparação de Letras e Shows para Leitura Offline</li>
