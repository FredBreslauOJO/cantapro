...
const ProtectedRoute = ({ children }) => {
  ...
  if (isLoadingAuth) return <div className="min-h-screen flex items-center justify-center font-bold uppercase tracking-widest text-black">Carregando...</div>;
  if (!isAuthenticated) return <Navigate to="/login" replace />;
  return children;
};

// NOVO: GUARDA-COSTAS PARA ROTAS EXCLUSIVAS PRO
const ProRoute = ({ children }) => {
  const { isAuthenticated, isLoadingAuth, plan } = useAuth();
  if (isLoadingAuth) return <div className="min-h-screen flex items-center justify-center font-bold uppercase tracking-widest text-black">Carregando...</div>;
  if (!isAuthenticated) return <Navigate to="/login" replace />;
  
  // Se tentar acessar e não for PRO, joga de volta pra home
